...
| Server-to-server | Mandy API keys (see below) |
...
For server-to-server access, Mandy issues self-contained signed API
keys:
...
- **Rate limits** apply per caller address and flow.
...
- Optional **signed identity**: your backend vouches for a visitor
  with an HMAC-signed token; Mandy verifies it against a
...
Mandy covers the working surface of authentication, authorization,
