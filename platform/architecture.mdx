...
```mermaid
flowchart LR
    D["Doppler<br/>(secrets)"] --> E1["/.env.dev/"]
    NS["infra/env.nonsecret.json<br/>(non-secret app config)"] --> GEN
    PJ["infra/ports.json<br/>(ports + URLs)"] --> GEN
    GEN["infra/scripts/generate.py<br/>&lt;env&gt;"] --> E2["/.env.&lt;env&gt;.generated/"]
    GEN --> CF["deploy/Caddyfile"]
    GEN --> CC["docker-compose.*.yml"]
    E1 --> SVC(["Services"])
    E2 --> SVC
```

Three environments: `dev`, `lab`, `sandbox`, `prod`. Doppler always wins
over `env.nonsecret.json` at runtime — **never duplicate keys between
them**.
