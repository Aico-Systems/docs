...
Keeping them distinct is central to Mandy's security model.
...
3. **Signed identity** — your backend signs the visitor's user ID
   ...
   token, and Mandy verifies it. This names *your* user without any
   Mandy login. When a signed identity appears, memory accumulated
...
allowlist, and per-address rate limits apply to each flow.
