...
Everything in one Kubernetes cluster (lab or cloud). The `make bootstrap-lab`
...
| Branch | Environment | Doppler config |
|---|---|---|
| `dev` | Dev | `dev` |
| `sandbox` | Sandbox | `stg` |
| `main` | Production | `prd` |
| `lab` | Optional local-VM | inherits `dev` |

Webhook-based CD (CI builds images → GHCR → webhook POST → server pulls
+ restarts). Server provisioning via cloud-init
(`deploy/cloud-init/aico-server.yml`).
...
```mermaid
flowchart LR
    DP["Doppler<br/>(secrets)"] --> ESO["External Secrets<br/>Operator"]
    NS["infrastructure/config/<br/>env.nonsecret.json"] --> GEN
    PJ["infrastructure/config/<br/>ports.json"] --> GEN
    GEN["gen-configmaps.py &lt;env&gt;"] --> CM["Kubernetes<br/>ConfigMaps"]
    ESO --> SEC["Kubernetes Secrets"]
    CM --> POD[/Pods (envFrom)/]
    SEC --> POD
```

Doppler wins over `env.nonsecret.json` at runtime. **Never duplicate
keys between them.**
