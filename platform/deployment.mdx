...
Everything in one `docker compose up`. Makefile auto-detects `/dev/kfd`
+ `rocminfo` and layers in `docker-compose.rocm.yml` when a ROCm GPU is
present.
...
```mermaid
flowchart LR
    DP["Doppler<br/>(secrets)"] --> ED["/.env.dev/"]
    NS["env.nonsecret.json"] --> GEN
    PJ["ports.json"] --> GEN
    GEN["generate.py &lt;env&gt;"] --> EG["/.env.<env>.generated/"]
    GEN --> CF["Caddyfile"]
