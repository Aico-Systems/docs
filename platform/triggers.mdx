...
The general-purpose trigger. Accepts a user JWT **or** a Mandy API
key. Modes:

- **Headless** — runs the flow without any live conversation surface
  (data processing, notifications, side effects).
- **Outbound call** — with a `destination` phone number, Mandy dials
...
<Info>
  Mandy has no built-in scheduler by design. Recurring or scheduled
